    ...
    if [[ -s $HOME/accuweather_conky/curr_cond_raw ]]; then
        ...
        sed -i -e 's/">//g' -e 's/-->//g' -e 's/\r$//g' -e 's/ i-alarm.*$//g' $HOME/accuweather_conky/curr_cond
            time=$(sed -n 1p $HOME/accuweather_conky/curr_cond)
        image=$(sed -n 2p $HOME/accuweather_conky/curr_cond)
            if [[ $time == day ]]; then
                sed -i 2s/$image/$(test_image_day $image)/ $HOME/accuweather_conky/curr_cond
            elif [[ $time == night ]]; then
                sed -i 2s/$image/$(test_image_night $image)/ $HOME/accuweather_conky/curr_cond
            fi
    ...
    if [[ -s $HOME/accuweather_conky/tod_ton_raw ]]; then
        ...
        sed -i -e 's/<span>.*$//g' -e 's/<\/span>//g' -e 's/\r$//g' -e 's/ i-alarm.*$//g' $HOME/accuweather_conky/tod_ton
            sed -i -e 's/Early AM/EARLY AM/' -e 's/Today/TODAY/' -e 's/Tonight/TONIGHT/' -e 's/Overnight/OVERNIGHT/' -e 's/Mon/MONDAY/' -e 's/Tue/TUESDAY/' -e 's/Wed/WEDNESDAY/' -e 's/Thu/THURSDAY/' -e 's/Fri/FRIDAY/' -e 's/Sat/SATURDAY/' -e 's/Sun/SUNDAY/' $HOME/accuweather_conky/tod_ton
        time=$(sed -n 1p $HOME/accuweather_conky/tod_ton)
        image=$(sed -n 2p $HOME/accuweather_conky/tod_ton)
            if [[ $time == TODAY ]]; then
                sed -i 2s/$image/$(test_image_day $image)/ $HOME/accuweather_conky/tod_ton
            elif [[ $time == TONIGHT || $time == OVERNIGHT || $time == "EARLY AM" ]]; then
                sed -i 2s/$image/$(test_image_night $image)/ $HOME/accuweather_conky/tod_ton
            sed -i 3a- $HOME/accuweather_conky/tod_ton
            fi
        for (( i=7; i<=22; i+=5 ))
              do
              image=$(sed -n "${i}"p $HOME/accuweather_conky/tod_ton)
                  sed -i ${i}s/$image/$(test_image_day $image)/ $HOME/accuweather_conky/tod_ton
              done
